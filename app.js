...
app.get('/', (req, res) => {
    res.send('Hello World Aniket! suces hogya')
  })
  app.post('/redux', (req,res) => {
    res.send("redux success")
  })
    ...
    // Write into DB
        try {
            ...
            if(formDs) {
                return res.send({
                    status: 401,
                    message: "email already taken please use another name anirrrlk"
                })
            }
        }
catch(err){
...
app.post('/Card', async(req, res) => {
    console.log(req.body);
    const {  name, phone, memberno } = req.body;

    if( !name || !phone ||!memberno) {
        return res.send({
            status: 400,
            message: "Missing data from 1",
            data: req.body
...
app.post('/cardfind', async(req, res) => {
    let phone = req.body.phone;
    let search
    try {
         search= await FormModel.findOne({phone:phone})
        if(!search){
            return res.send({
